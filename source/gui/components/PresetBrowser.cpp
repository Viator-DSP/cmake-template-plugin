...
void PresetBrowser::populateSubMenu(std::vector<juce::File>& presets, const juce::StringArray& fileNames, juce::PopupMenu& menu, int offset)
{
...
void PresetBrowser::addFactoryFiles(const void* wav, size_t wavSize, const juce::String& fileName, PresetType presetType)
{
...
void PresetBrowser::checkIfPresetIsDirty()
{
    // get the xml file
    juce::File file (_xmlFilePaths[_presetMenu.getSelectedItemIndex()]);
        ...
        if (rootElement != nullptr && rootElement->hasTagName(appName))
        {
            ...
            while (paramElement != nullptr)
            {
                for (int i = 0; i < paramElement->getNumAttributes(); ++i)
                {
                    ...
                    float savedParamValue = paramElement->getDoubleAttribute(paramName);
                    float currentParamValue = audioProcessor.get_tree_state().getRawParameterValue(paramName)->load();
                    bool isDirty = (int)savedParamValue != (int)currentParamValue;
                    audioProcessor.getVariableTree().setProperty("presetDirty", isDirty, nullptr);
                    if (isDirty)
                    {
                        return;
                    }
