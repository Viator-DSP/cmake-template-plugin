...
#if USING_RUST
  auto result_from_rust = saturatorrustlib::my_rust_function(3, 9);
...
juce::AudioProcessorValueTreeState::ParameterLayout PluginProcessor::createParameterLayout()
{
    ...
    for (int i = 0; i < _parameterMap.getSliderParams().size(); i++)
    {
        ...
        if (param.isInt == tuc::ParameterData::SliderParameterData::NumericType::kInt || param.isSkew == tuc::ParameterData::SliderParameterData::SkewType::kSkew)
        {
            auto range = juce::NormalisableRange<float>(param.min, param.max);

            if (param.isSkew == tuc::ParameterData::SliderParameterData::SkewType::kSkew)
            {
